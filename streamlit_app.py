...
st.set_page_config(page_title="Lévy Copula – Tail Dependence Engine", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 Lévy Copula – Tail Dependence Engine</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Empirical tail dependence (Lévy copula) for ETFs | Measures joint extreme moves</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Lévy Copula")
...
st.sidebar.markdown(f"**Tail quantile:** {config.TAIL_QUANTILE}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'levy_copula_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Tail dependence: {etf['tail_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Tail Dependence"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Tail Dependence", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📊 Top ETFs by Average Tail Dependence (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Lévy copula** models the dependence structure of extreme returns (tail dependence).
        - We estimate pairwise lower tail dependence: probability that both ETFs are in their 5% worst days.
        - The per‑ETF score is the **average tail dependence** with all other ETFs.
        - High score → the ETF tends to crash together with many others → systemic risk.
        - Low score → the ETF crashes independently → potential diversification benefit.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the tail dependence scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Lévy Copula | Empirical tail dependence for ETFs")
